...
def _is_tied_lm_head_and_embed(model: torch.nn.Module) -> bool:
    ...
    if not hasattr(embed, "weight") or not hasattr(lm_head, "weight"):
        return False
    try:
...
def fuse_layer_norms(model):
    ...
    # For tied embeddings (e.g., Qwen/Llama with tied lm_head and embed_tokens),
    # fusing model.norm into lm_head would also mutate embed_tokens and corrupt inputs.
    if not _is_tied_lm_head_and_embed(model):
        fuse_ln_linear(
            model.model.norm,
            [model.lm_head],
        )
