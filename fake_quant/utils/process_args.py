...
def parser_gen():
    ...
    parser.add_argument(
        ...
        type=str,
        default="a_cov",
        choices=["a_cov", "wa_cov"],
        help=(
            "Covariance objective for basis construction. "
            "'a_cov': activation-only baseline; "
            "'wa_cov': weight-activation fused objective "
            "(self_attn/mlp; K/V extension controlled by --use_kv_wa_cov); "
        ),
    )
    parser.add_argument(
        "--use_pre_fuse_lambda_compensation",
        type=int,
        default=0,
        choices=[0, 1],
        help=(
            "Whether wa_cov estimates lambda in pre-fuse-equivalent space "
            "with diagonal congruence correction (1: enable, 0: disable)."
        ),
    )
    parser.add_argument(
        "--use_kv_wa_cov",
        ...
        choices=[0, 1],
        help=(
            "Whether to apply wa_cov-style objective to K/V basis construction "
            "(1: enable kv innovation, 0: keep default a_cov for K/V)."
        ),
    )
    parser.add_argument(
        "--down_proj_blocksize",
    ...
    parser.add_argument('--lm_eval_batch_size', type=int, default=8, help='Batch size for evaluating with lm eval harness.')
    parser.add_argument(
        '--tasks',
        nargs='+',
        default=["piqa", "arc_easy", "arc_challenge", "winogrande", "boolq", "social_iqa"],
    )
