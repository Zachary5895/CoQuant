...
def rearrange_o_proj(layer, high_bits_length, low_bits_length, head_dim, training):
    ...
    new_column_order = torch.cat((columns_to_beginning, remaining_columns, columns_to_end)) #重排，原本是0...2047，现在是0,...55,64...119,...20xx,56,57,58...
...
def fuse_basis_shared(model, args):
    ...
    for idx, layer in enumerate(tqdm.tqdm(layers, unit="layer", desc="Rotating")):
        ...
        rotate_attention_inputs(layers[idx], U_attn) #旋转QKV
...
class QKRotationWrapper(torch.nn.Module):
    ...
    def forward(self, *args, **kwargs):
        ...
        if self.k_had:
            ...
            def _resolve_rotation_for_heads(rotation, target_heads, q_heads, k_heads, tensor_name):
                if rotation.ndim == 2:
                    return rotation
                if rotation.ndim != 3:
                    raise RuntimeError(
                        f"Unsupported {tensor_name} rotation ndim={rotation.ndim}, expected 2 or 3"
                    )
                if rotation.shape[0] == target_heads:
                    return rotation
                if rotation.shape[0] == 1:
                    return rotation.expand(target_heads, -1, -1)
                if (
                    tensor_name == "q"
                    and rotation.shape[0] == k_heads
                    and q_heads % k_heads == 0
                ):
                    return rotation.repeat_interleave(q_heads // k_heads, dim=0)
                raise RuntimeError(
                    f"Incompatible {tensor_name} rotation heads: got {rotation.shape[0]}, "
            ...
            q_rotation = _resolve_rotation_for_heads(
                self.k_rotation.to(q), q_heads, q_heads, k_heads, "q"
            )
            q = torch.matmul(q, q_rotation)
            self.pre_rotation_quantizer.free()

            self.pre_rotation_quantizer.find_params(k)
            k = self.pre_rotation_quantizer(k).to(dtype)
            k_rotation = _resolve_rotation_for_heads(
                self.k_rotation.to(k), k_heads, q_heads, k_heads, "k"
            )
            k = torch.matmul(k, k_rotation)
